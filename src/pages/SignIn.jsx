...
const SignIn = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error('Sorry: Email or Password was wrong.')
    }
  }
  return (
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className='my-5'>
          ...
          <div className='my-2 w-full relative rounded-2xl shadow-xl'>
            <input onChange={(e) => setEmail(e.target.value)} className="w-full p-2 border rounded-2xl outline-none" required id='email' type='email' placeholder="Enter Email..."/>
        ...
        <button className='sign-btn' type='submit'><span className='hover:animate-pulse'>Sign in</span></button>
        
      </form>
      <p className='my-3 text-gray-500 dark:text-gray-400 text-sm'>
        Don't have an account? <Link to='/signup' className='text-slate-800 font-semibold dark:text-blue-200 hover:animate-pulse'>Register</Link>
      </p>
    </div>
  )
}
